...
free_sigma0_data = linspace(0, 1, num);
...
for i = 1:num
    ...
    %k=1e-5;
    free_sigma0 = -(2*E*R*T/Vs * log(Rd/(k*c_eq0) + 1)) ^0.5;
    free_sigma0_data(i) = free_sigma0;
end
...
%%%%%%% Legend %%%%%%
legend([p1 p2], 'Contact surface','Free surface','FontSize',16,'Interpreter','latex')
...
text(3e3,2e-8, '$k = 1 \times 10^{-5}$ m/s','FontSize',16,'Interpreter','latex')
%text(1e4,2e-9, '$r_0 = 1 \times 10^{-3}$ m','FontSize',16,'Interpreter','latex')
